...
function RefereeSchedulePage() {
    ...
    const filteredGames = games.filter(game => {
        // Filter logic
        if (filter === 'my-games') {
            if (!currentPlayer) return false; // If no linked player, show nothing for "my games"
            return game.referee1Id === currentPlayer.id || game.referee2Id === currentPlayer.id;
        }
                    ...
                    <table className="games-table">
                        ...
                        <tbody>
                            {filteredGames.map((game, index) => {
                                ...
                                const ref1 = players.find(p => p.id === game.referee1Id);
                                const ref2 = players.find(p => p.id === game.referee2Id);
                                ...
                                return (
                                    <tr key={game.id} className={isNewWeek ? 'week-separator' : ''}>
                                        ...
                                        <td>{game.rink || 'TBD'}</td>
                                        <td>
                                            <div>{ref1 ? `${ref1.firstName} ${ref1.lastName}` : '-'}</div>
                                            <div>{ref2 ? `${ref2.firstName} ${ref2.lastName}` : '-'}</div>
                                        </td>
                                    </tr>
                                );
                            })}
